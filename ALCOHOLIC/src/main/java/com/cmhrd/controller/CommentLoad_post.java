...
public class CommentLoad_post extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		dto.setPost_idx(idx);
		System.out.println("받아온 인덱스 번호"+idx);
		List<post_commentDTO> list = dao.loadcomment_post(dto);
        ...
        for (int i = 0; i < list.size(); i++) {
        	post_commentDTO post = list.get(i);
        	json.append("{");
